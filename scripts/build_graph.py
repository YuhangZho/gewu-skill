...
# 主状态只表达进度；学习路径用 track 表达。
DONE_STATUS = '已完成'
IN_PROGRESS_STATUS = '学习中'
TODO_STATUS = '待学'
VALID_STATUSES = {DONE_STATUS, IN_PROGRESS_STATUS, TODO_STATUS}
VALID_TRACKS = {'快轨', '标准轨', '深轨'}

def normalize_status(raw):
    s = str(raw or TODO_STATUS).strip()
    return s if s in VALID_STATUSES else TODO_STATUS

def normalize_track(raw_track):
    track = str(raw_track or '').strip()
    return track if track in VALID_TRACKS else ''

def is_done(note):
    return (note.get('status') or '').strip() == DONE_STATUS
...
def collect(vault):
    ...
    for root, dirs, files in os.walk(vault):
        ...
        for fn in files:
            ...
            raw_status = fm.get('status') or TODO_STATUS
            status = normalize_status(raw_status)
            track = normalize_track(fm.get('track'))
...
def build_graph(notes):
    ...
    # outer fringe（知识空间理论 KST）：前置全部已完成、自身未完成 → ready=可学的"下一站"
    def is_ready(n):
        if is_done(n):
            return False
        for raw in n['prereqs']:
            src = index.get(str(raw).strip().lower())
            if src is None or not is_done(notes[src]):
                return False
        return True
    # 大类中心节点
...
const PALETTES = {
  modern:{cat:["#7aa2f7","#9ece6a","#e0af68","#bb9af7","#f7768e","#2ac3de","#ff9e64","#73daca","#c0caf5"],
          status:{"已完成":"#9ece6a","学习中":"#56b6c2","待学":"#565f73"}},
  ink:{cat:["#2e5b7c","#b83a2e","#5a8f6d","#9c6b3f","#6a5a7d","#c89a3c","#3f6f8f","#a23b53","#4a5a66"],
       status:{"已完成":"#5a8f6d","学习中":"#5a86a0","待学":"#9a8f78"}}};
